...
서비스 즉시 메모리에 임시적으로 생성되며 서비스되지 않을 때 자동으로 없어진다. ::
   ...
   <MP4HLS Status="Inactive" Keyword="mp4hls" Path="absolute">
...
-  ``<MP4HLS>``
   ...
   - ``Path (기본: absolute)`` m3u8 파일 리소스 경로 표현 ::
...
-  ``<Encrypt>``
   ...
   - ``Keyword (기본: segment.key)`` 암호화 Key 키워드

-  ``<Token>`` 암호화 Key의 시드 값으로 사용할 문자열을 입력한다. 
...
생성된 인덱스 파일은 다음과 같이 암호화 Key 정보를 ``#EXT-X-KEY`` 속성으로 명시한다. :: 
...
MP3파일을 HLS(HTTP Live Streaming)로 서비스한다. ::
   ...
   <MP3HLS Status="Inactive" Keyword="mp3hls" SegmentType="TS" InsertPCR="OFF" Path="absolute">
      <Index Ver="3" Alternates="off">index.m3u8</Index>
      <Sequence>0</Sequence>
      <Duration>10</Duration>
      <AlternatesName>playlist.m3u8</AlternatesName>
